...
access(all) contract PostNFT : SimpleNFT{
    ...
    access(all) resource NFT: SimpleNFT.DisplayableNFT, NonFungibleToken.NFT{
        ...
        access(all) fun resolveView(_ view: Type): AnyStruct? {
            switch view {
                ...
                case Type<MetadataViews.NFTCollectionData>(): return PostNFT.getCollectionData()
                case Type<MetadataViews.NFTCollectionDisplay>(): return PostNFT.getCollectionDisplay()
            }
            return nil
        }

        access(all) fun createEmptyCollection(): @{NonFungibleToken.Collection} {
            return <- PostNFT.createEmptyCollection(nftType: Type<@PostNFT.NFT>())
        }
    }
    ...
    access(all) resource Minter {
        access(all) fun mintNFT(metadata: {String: AnyStruct}, receiver : &{NonFungibleToken.Receiver}){
            ...
            emit Minted(id: nft.id, uuid:nft.uuid, to: receiver.owner?.address, type: Type<@PostNFT.NFT>().identifier)
    ...
    access(all) fun createEmptyCollection(nftType: Type): @{NonFungibleToken.Collection} {
        return <- PostNFT.createEmptyUniversalCollection()
    }

    init() {
        let minter <- create Minter()
        self.nftType= Type<@PostNFT.NFT>() //we cannot have generics so we make a poor mans generics

        self.minterPath=/storage/postNFTMinter
