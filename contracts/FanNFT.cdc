...
access(all) contract FanNFT : SimpleNFT{
    ...
    access(all) resource NFT: SimpleNFT.DisplayableNFT, NonFungibleToken.NFT{
        ...
        access(all) fun resolveView(_ view: Type): AnyStruct? {
            switch view {
                ...
                case Type<MetadataViews.NFTCollectionData>(): return FanNFT.getCollectionData()
                case Type<MetadataViews.NFTCollectionDisplay>(): return FanNFT.getCollectionDisplay()
            }
            return nil
        }

        access(all) fun createEmptyCollection(): @{NonFungibleToken.Collection} {
            return <- FanNFT.createEmptyCollection(nftType: Type<@FanNFT.NFT>())
        }
    }
    ...
    access(all) resource Minter {
        access(all) fun mintNFT(metadata: {String: AnyStruct}, receiver : &{NonFungibleToken.Receiver}){
            ...
            emit Minted(id: nft.id, uuid:nft.uuid, to: receiver.owner?.address, type: Type<@FanNFT.NFT>().identifier)
    ...
    access(all) fun createEmptyCollection(nftType: Type): @{NonFungibleToken.Collection} {
        return <- FanNFT.createEmptyUniversalCollection()
    }

    init() {
        let minter <- create Minter()
        self.nftType= Type<@FanNFT.NFT>() //we cannot have generics so we make a poor mans generics

        self.minterPath=/storage/fanNFTMinter
